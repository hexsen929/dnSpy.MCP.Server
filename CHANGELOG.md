...
### Fixed
- `list_assembly_references` and `remove_assembly_reference` now include assembly refs that are only anchored through `ExportedType` type-forwarders, so they stay consistent with `add_assembly_reference`.
