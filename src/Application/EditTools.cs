...
namespace dnSpy.MCP.Server.Application {
	...
	public sealed class EditTools {
		...
		public EditTools(IDocumentTreeView documentTreeView, IDsDocumentService documentService, IDecompilerService decompilerService) {
	...
	/// <summary>
	/// Lists all custom attributes declared on the assembly manifest ([assembly: ...]).
	/// Arguments: assembly_name
	/// </summary>
	public CallToolResult ListAssemblyAttributes(Dictionary<string, object>? arguments) {
		if (arguments == null)
			throw new ArgumentException("Arguments required");
		if (!arguments.TryGetValue("assembly_name", out var asmNameObj))
			throw new ArgumentException("assembly_name is required");
		...
		var attrs = assembly.CustomAttributes
			.Select((ca, i) => new {
				Index = i,
				TypeFullName = ca.AttributeType?.FullName ?? "",
				TypeName = ca.AttributeType?.Name?.String ?? "",
				CtorArgs = ca.ConstructorArguments.Select(a => a.Value?.ToString() ?? "null").ToList(),
				NamedArgs = ca.NamedArguments.Select(a => new { Name = a.Name.String, Value = a.Argument.Value?.ToString() ?? "null" }).ToList()
			})
			.ToList();
	...
	/// <summary>
	/// Removes one or more custom attributes from the assembly manifest ([assembly: ...]).
	/// Arguments: assembly_name, attribute_type_name (simple or full), index (optional 0-based — removes specific match when multiple present)
	/// Omitting index removes ALL matching attributes.
	/// Changes are in-memory until save_assembly is called.
	/// </summary>
	public CallToolResult RemoveAssemblyAttribute(Dictionary<string, object>? arguments) {
		if (arguments == null)
			throw new ArgumentException("Arguments required");
		if (!arguments.TryGetValue("assembly_name", out var asmNameObj))
			throw new ArgumentException("assembly_name is required");
		...
		var attrTypeName = attrTypeObj.ToString() ?? "";

		// Find matching entries (match on simple name or full name, case-insensitive)
		var matches = assembly.CustomAttributes
			.Select((ca, i) => (ca, i))
			.Where(x => {
				var simpleName = x.ca.AttributeType?.Name?.String ?? "";
				var fullName   = x.ca.AttributeType?.FullName ?? "";
				return simpleName.Equals(attrTypeName, StringComparison.OrdinalIgnoreCase)
				    || fullName.Equals(attrTypeName, StringComparison.OrdinalIgnoreCase);
			})
			.ToList();

		if (matches.Count == 0)
			throw new ArgumentException(
				$"No custom attribute matching '{attrTypeName}' found on assembly '{asmNameObj}'. " +
				"Use list_assembly_attributes to see all assembly-level attributes.");

		var removed = new List<string>();

		if (arguments.TryGetValue("index", out var indexObj) && indexObj != null &&
		    int.TryParse(indexObj.ToString(), out var idx)) {
			// Remove a specific match by its position in the match list
			if (idx < 0 || idx >= matches.Count)
				throw new ArgumentException($"index {idx} is out of range (0–{matches.Count - 1})");
			var (ca, _) = matches[idx];
			assembly.CustomAttributes.Remove(ca);
			removed.Add(ca.AttributeType?.FullName ?? attrTypeName);
		}
		else {
			// Remove all matches — iterate from the end to avoid shifting indices
			foreach (var (ca, _) in matches.OrderByDescending(x => x.i)) {
				assembly.CustomAttributes.Remove(ca);
				removed.Add(ca.AttributeType?.FullName ?? attrTypeName);
			}
		}
		...
		return new CallToolResult {
			Content = new List<ToolContent> { new ToolContent {
				Text = $"Removed {removed.Count} attribute(s) from assembly '{asmNameObj}':\n" +
				       string.Join("\n", removed.Select(r => "  • " + r)) +
				       "\nNote: Use save_assembly to persist changes to disk."
			} }
		};
	}
	...
	/// <summary>
	/// Lists all P/Invoke (DllImport) declarations in a type or the entire assembly.
	/// Returns the managed method name, token, DLL name, and native function name.
	/// Arguments: assembly_name, type_full_name (optional — omit to scan all types)
	/// </summary>
	public CallToolResult ListPInvokeMethods(Dictionary<string, object>? arguments) {
		...
		if (!string.IsNullOrEmpty(typeNameStr)) {
			// Single-type scan (original behaviour)
			var type = FindTypeInAssemblyAll(assembly, typeNameStr);
			if (type == null)
				throw new ArgumentException($"Type not found: {typeNameStr}");
			...
			// Group by DLL for easier scanning
			var byDll = pinvokes
				.GroupBy(p => p.DllName)
				.Select(g => new { Dll = g.Key, Count = g.Count() })
				.OrderByDescending(g => g.Count)
				.ToList();

			var result = JsonSerializer.Serialize(new {
				Scope = $"[assembly: {asmNameObj}]",
				PInvokeCount = pinvokes.Count,
				ByDll = byDll,
				Methods = pinvokes
			}, new JsonSerializerOptions { WriteIndented = true });
